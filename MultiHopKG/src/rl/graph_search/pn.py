...
class GraphSearchPolicy(nn.Module):
    ...
    def transit(self, e, obs, kg, use_action_space_bucketing=True, merge_aspace_batching_outcome=False):
        ...
        Q = kg.get_relation_embeddings(q) # (batch_size, relation_dim)
        H = self.path[-1][0][-1, :, :] # self.path[-1][0] is last step h_n, so H is (batch_size, history_dim)
        if self.relation_only: 
        ...
        X = self.W1Dropout(X)
        X = self.W2(X)
        X2 = self.W2Dropout(X) # (batch_size, action_dim)
    ...
    def initialize_path(self, init_action, kg):
        ...
        self.path = [self.path_encoder(init_action_embedding, (init_h, init_c))[1]] # list of (h_n, c_n)
                                                                                    # h_n (history_num_layers, batch, history_dim)
    ...
    def update_path(self, action, kg, offset=None):
        ...
        if self.relation_only_in_path:
            ...
            action_embedding = self.get_action_embedding(action, kg) # (batch_size, action_dim)
    ...
    def get_action_space(self, e, obs, kg):
        """Get action space for entity `e`.

        Get action sapce for entity `e` and then apply mask.
        """
        r_space, e_space = kg.action_space[0][0][e], kg.action_space[0][1][e] # (batch_size, action_space)
    ...
    def get_ground_truth_edge_mask(self, e, r_space, e_space, e_s, q, e_t, kg):
        """Mask ground truth edge.

        When e==e_s, an action in (r_space, e_space) may be (q, e_t), we should forbid the model 
        just use this path to reason. the same as inv relation.
        """
