...
def beam_search(pn, e_s, q, e_t, kg, num_steps, beam_size, return_path_components=False):
    ...
    def top_k_action(log_action_dist, action_space):
        ...
        next_r = ops.batch_lookup(r_space.view(batch_size, -1), action_ind).view(-1) # [batch_size*k]
        next_e = ops.batch_lookup(e_space.view(batch_size, -1), action_ind).view(-1) # [batch_size*k]
        ...
        return (next_r, next_e), log_action_prob, action_offset # [batch_size*k]
    ...
    beam_search_output['pred_e2s'] = action[1].view(batch_size, -1) # (batch_size, k)
    beam_search_output['pred_e2_scores'] = log_action_prob.view(batch_size, -1) # (batch_size, k)
    if kg.args.save_beam_search_paths:
