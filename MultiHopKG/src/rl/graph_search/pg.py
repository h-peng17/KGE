...
class PolicyGradient(LFramework):
    ...
    def loss(self, mini_batch):
        ...
        entropy = torch.cat([x.unsqueeze(1) for x in action_entropy], dim=1).mean(dim=1) # (batch_size)
    ...
    def rollout(self, e_s, q, e_t, num_steps, visualize_action_probs=False):
        ...
        return {
            'pred_e2': pred_e2, # (batch_size, sample_size) 
            'log_action_probs': log_action_probs, # list of log action probs. [(batch_size, sample_size), ...]
            'action_entropy': action_entropy, # list of policy entropy. [(batch_size), ...]
            'path_trace': path_trace, # list of action. [(r, e), ...]. r/e is (batch_size, sample_size)
    ...
    def predict(self, mini_batch, verbose=False):
        kg, pn = self.kg, self.mdl
        e1, e2, r = self.format_batch(mini_batch) # (batch_size)
        ...
        return pred_scores # (batch_size, num_entities)
